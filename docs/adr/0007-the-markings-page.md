...
**Amended — 2026-09-24 (#151):** the active row's own anatomy — its clock becomes the time field, its alias is text until it is active, and Delete becomes a trash glyph. Also under the Decision's key bullet, below; the prototype that settled it is on the `prototype/markings-rows` branch.
