...
/**
 * Keyboard navigation over markers: the pure policy behind the ↑/↓ jumps.
 * Both functions take markers in time order — the shape `deriveLabels`
 * returns — and return the target marker, or null when there is nothing to
 * jump to. They never mutate; jumping is a read. The A–Z letter jumps left
 * with ADR-0005: labels restart per movement, so a single letter key could no
 * longer name one marker unambiguously.
 *
 ...
 * markers sharing one time is skipped past as a group.
 */
